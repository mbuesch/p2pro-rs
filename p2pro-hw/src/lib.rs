...
impl<H: CameraConfigHwAccess> CameraConfig<H> {
    ...
    async fn control_in(&mut self, mailbox: u16, buf: &mut [u8]) -> ah::Result<()> {
        self.dev_accessed = true;
        let read = self
            .handle
            .read_control(
    ...
    async fn status(&mut self) -> ah::Result<u8> {
        let mut buf = [0_u8; 1];
    ...
    async fn wait_ready(&mut self) -> ah::Result<()> {
    ...
    async fn write_block(&mut self, block: &[u8]) -> ah::Result<()> {
        let mut offset = 0;
    ...
    async fn standard_read_u16be(&mut self, code: u16) -> ah::Result<u16> {
    ...
    /// Long read.
    async fn long_read(&mut self, code: u16, p1: u16, p2: u32, len: usize) -> ah::Result<Vec<u8>> {
        self.control_out(MAILBOX_HEADER_B, &long_command_header(code, p1, p2))
            .await?;
    ...
    pub async fn reset_to_rom(mut self) -> ah::Result<()> {
    ...
    pub async fn device_info_summary(&mut self) -> ah::Result<Vec<String>> {
        let items = [
    ...
    pub async fn set_palette(&mut self, palette: Palette) -> ah::Result<()> {
        self.standard_write(
    ...
    pub async fn palette(&mut self) -> ah::Result<Palette> {
        let data = self
    ...
    pub async fn shutter_vtemp(&mut self) -> ah::Result<u16> {
        self.standard_read_u16be(CMD_SHUTTER_VTEMP).await
    }

    /// Read the current temperature-related raw value.
    pub async fn current_vtemp(&mut self) -> ah::Result<u16> {
    ...
    pub async fn emissivity(&mut self) -> ah::Result<f32> {
        Ok(f32::from(self.tpd_get(TpdParam::Emissivity).await?) / 128.0)
    }

    /// Set the object emissivity, clamped to 0.0 - 1.0.
    pub async fn set_emissivity(&mut self, emissivity: f32) -> ah::Result<()> {
    ...
    pub async fn distance(&mut self) -> ah::Result<f32> {
        Ok(f32::from(self.tpd_get(TpdParam::Distance).await?) / 163.835)
    }

    /// Set the object distance in metres (0 - ~200 m).
    pub async fn set_distance(&mut self, metres: f32) -> ah::Result<()> {
    ...
    pub async fn atmospheric_transmittance(&mut self) -> ah::Result<f32> {
    ...
    pub async fn set_atmospheric_transmittance(&mut self, transmittance: f32) -> ah::Result<()> {
